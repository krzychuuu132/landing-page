...
export class OpinionsSlide {
  ...
  renderStars(value: number) {
    const range = (start: number, end: number) => {
      return new Array(end - start + 1).fill(0).map((_, index) => start + index);
    };
    const starsElementArray = range(1, value);
    return starsElementArray.map(_ => (
  ...
  render() {
    const { name, description, stars } = this.opinionsSlideData;
    ...
    const html = `
        <div class="opinions__opinion">
            <div class="heading">
                ...
                <div class="stars">
                    ${this.renderStars(parseInt(stars[0]))}
                </div>
            </div>
